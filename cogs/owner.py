...
class Owner(commands.Cog, name="owner"):
    ...
    @commands.command(
        name="disconnect",
        description="sussy bot commits"
    )
    @checks.is_owner()
    async def disconnect(self, ctx):
        await ctx.send("Lmao sussy amongus")
        await ctx.send("Bye I will now commit")
        await ctx.send("https://tenor.com/zh-TW/view/among-us-twerk-fast-ass-shake-gif-20107138")
        await self.bot.close()

    @commands.command(
        name="die",
        description="Tell Imposter to die"
    )
    @checks.is_owner()
